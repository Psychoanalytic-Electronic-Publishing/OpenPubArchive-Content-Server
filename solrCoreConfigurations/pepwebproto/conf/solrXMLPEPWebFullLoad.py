...
def main():
    ...
    print """PEP Solr Full Text Data Loader
             ...
             2019-05-15 - Working, first version 0.1.15 for first version schema - nrs
        """

    parser = OptionParser(usage="%prog [options] - PEP Solr Full Text Data Loader", version="%prog ver. 0.1.15")
    ...
    parser.add_option("-d", "--dataroot", dest="rootFolder", default=r"C:\solr-8.0.0\server\solr\pepwebproto\sampledata\_PEPA1",
                      help="Root folder where input data is located")
    parser.add_option("-c", "--corename", dest="coreName", default="pepwebproto",
                      help="the Solr corename (holding the collection) to connect to, i.e., where to send data")
    ...
    print "Ready to import full-text records from %s files at path: %s" % (len(filenames), options.rootFolder)
    bibTotalReferenceCount = 0

    #----------------------------------------------------------------------------------------
    #Data Cases - quick testing for a specific structure, just uncomment filenames and set filename there
    #Test File: Example Dream
    #filenames = [r"C:\solr-8.0.0\server\solr\pepwebproto\sampledata\_PEPA1\_PEPArchive\APA\063.2015\APA.063D.0769A(bEXP_ARCH1).XML"]
    #----------------------------------------------------------------------------------------
    ...
    for n in filenames:
        ...
        try:
            ...
            if artIDFromFile != art_id:
                print "Warning: File name ID tagged and artID disagree.  %s vs %s" % (art_id, artIDFromFile)
        except Exception, err:
            print "Warning: issue reading file's article id."
