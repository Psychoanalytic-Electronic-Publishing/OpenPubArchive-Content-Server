...
CREATE TABLE `api_products`  (
  ...
  INDEX `basecode`(`basecode`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 7779 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `api_subscriptions`  (
  ...
  CONSTRAINT `subscribedProduct` FOREIGN KEY (`product_id`) REFERENCES `api_products` (`product_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `api_user`  (
  ...
  `user_agrees_date` datetime(0) NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(0),
  `user_agrees_to_tracking` tinyint(1) NULL DEFAULT 0,
  `user_agrees_to_cookies` tinyint(1) NULL DEFAULT 0,
  `added_date` datetime(0) NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(0),
  `last_update` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
  PRIMARY KEY (`user_id`) USING BTREE,
  UNIQUE INDEX `unique_username`(`username`) USING BTREE,
  INDEX `idxUsername`(`username`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 16 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = 'A list of all authorized users, and the NOT_LOGGED_IN user, with ID=0' ROW_FORMAT = Dynamic;
...
CREATE TABLE `api_user_ip_ranges`  (
  ...
  CONSTRAINT `api_user_ip_ranges_ibfk_1` FOREIGN KEY (`user_id`, `subsystem_id`) REFERENCES `user_subsystem` (`user_id`, `subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- ----------------------------
-- Table structure for join_group_to_group
-- ----------------------------
DROP TABLE IF EXISTS `join_group_to_group`;
CREATE TABLE `join_group_to_group`  (
  `group_join_id` int(11) NOT NULL AUTO_INCREMENT,
  `group_parent_id` int(11) NULL DEFAULT NULL,
  `group_child_id` int(11) NULL DEFAULT NULL,
  `description` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`group_join_id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 10 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for join_groups_to_products
-- ----------------------------
DROP TABLE IF EXISTS `join_groups_to_products`;
CREATE TABLE `join_groups_to_products`  (
...
-- ----------------------------
-- Table structure for join_products_to_productbase
-- ----------------------------
DROP TABLE IF EXISTS `join_products_to_productbase`;
CREATE TABLE `join_products_to_productbase`  (
  `product_id` int(11) NOT NULL,
  `basecode` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  PRIMARY KEY (`product_id`, `basecode`) USING BTREE,
  INDEX `fk_basecode`(`basecode`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `referrer_urls`  (
  ...
  INDEX `idxUserId`(`user_id`) USING BTREE,
  INDEX `FK_referrer_urls_user_subsystem`(`user_id`, `subsystem_id`) USING BTREE,
  CONSTRAINT `referrer_urls_ibfk_1` FOREIGN KEY (`user_id`, `subsystem_id`) REFERENCES `user_subsystem` (`user_id`, `subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- ----------------------------
-- Table structure for xxx___private_api_user
-- ----------------------------
DROP TABLE IF EXISTS `xxx___private_api_user`;
CREATE TABLE `xxx___private_api_user`  (
  `user_id` int(11) NOT NULL AUTO_INCREMENT,
  `username` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `password` varchar(512) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `company` varchar(125) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL,
  ...
  UNIQUE INDEX `unique_username`(`username`) USING BTREE,
  INDEX `idxUsername`(`username`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 119128 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = 'A list of all authorized users, and the NOT_LOGGED_IN user, with ID=0' ROW_FORMAT = Dynamic;
...
CREATE TABLE `xxx_gvpi_product`  (
  ...
  CONSTRAINT `xxx_gvpi_product_ibfk_1` FOREIGN KEY (`subsystem_id`) REFERENCES `subsystem` (`subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `xxx_gvpi_user_ip_addresses`  (
  ...
  INDEX `idxUserId`(`user_id`) USING BTREE,
  INDEX `FK_ip_addresses_user_subsystem`(`user_id`, `subsystem_id`) USING BTREE,
  CONSTRAINT `xxx_gvpi_user_ip_addresses_ibfk_1` FOREIGN KEY (`user_id`, `subsystem_id`) REFERENCES `user_subsystem` (`user_id`, `subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB AUTO_INCREMENT = 28533 CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
...
-- ----------------------------
-- View structure for vw_grouped_products
-- ----------------------------
DROP VIEW IF EXISTS `vw_grouped_products`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_grouped_products` AS select `api_product_groups`.`group_id` AS `group_id`,`api_product_groups`.`group_name` AS `group_name`,`join_groups_to_products`.`Description` AS `Description`,`join_groups_to_products`.`product_id` AS `product_id`,`vw_products_flattened`.`child_product_id` AS `child_product_id`,`vw_products_flattened`.`child_product` AS `child_product`,`vw_products_flattened`.`child_basecode` AS `child_basecode`,`vw_products_flattened`.`child_product_level` AS `child_product_level`,`vw_products_flattened`.`parent_product_id` AS `parent_product_id`,`vw_products_flattened`.`parent_product` AS `parent_product`,`vw_products_flattened`.`parent_basecode` AS `parent_basecode`,`vw_products_flattened`.`parent_product_level` AS `parent_product_level`,`vw_products_flattened`.`inherit_parent_metadata` AS `inherit_parent_metadata` from ((`join_groups_to_products` join `api_product_groups` on((`join_groups_to_products`.`group_id` = `api_product_groups`.`group_id`))) join `vw_products_flattened` on((`vw_products_flattened`.`parent_product_id` = `join_groups_to_products`.`product_id`))) union select `api_product_groups`.`group_id` AS `group_id`,`api_product_groups`.`group_name` AS `group_name`,`join_groups_to_products`.`Description` AS `Description`,`join_groups_to_products`.`product_id` AS `product_id`,`vw_products_flattened`.`child_product_id` AS `child_product_id`,`vw_products_flattened`.`child_product` AS `child_product`,`vw_products_flattened`.`child_basecode` AS `child_basecode`,`vw_products_flattened`.`child_product_level` AS `child_product_level`,`vw_products_flattened`.`parent_product_id` AS `parent_product_id`,`vw_products_flattened`.`parent_product` AS `parent_product`,`vw_products_flattened`.`parent_basecode` AS `parent_basecode`,`vw_products_flattened`.`parent_product_level` AS `parent_product_level`,`vw_products_flattened`.`inherit_parent_metadata` AS `inherit_parent_metadata` from ((`join_groups_to_products` join `api_product_groups` on((`join_groups_to_products`.`group_id` = `api_product_groups`.`group_id`))) join `vw_products_flattened` on((`vw_products_flattened`.`child_product_id` = `join_groups_to_products`.`product_id`))) order by `group_id`;
...
-- ----------------------------
-- View structure for vw_productsparent
-- ----------------------------
DROP VIEW IF EXISTS `vw_productsparent`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_productsparent` AS select `parent`.`product` AS `parent_product`,`api_products`.`product_level` AS `product_level`,`api_products`.`inherit_parent_metadata` AS `inherit_parent_metadata`,`api_products`.`id_type` AS `id_type`,`api_products`.`counter_service` AS `counter_service`,`api_products`.`product` AS `product` from (`api_products` left join `api_products` `parent` on((`parent`.`product_id` = `api_products`.`parent_product_id`))) order by `api_products`.`product_level`,`parent`.`product`,`api_products`.`product`;

-- ----------------------------
-- View structure for vw_products_by_group
-- ----------------------------
DROP VIEW IF EXISTS `vw_products_by_group`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_products_by_group` AS select `api_product_groups`.`group_id` AS `group_id`,`api_product_groups`.`group_name` AS `group_name`,`api_product_groups`.`level` AS `level`,`api_product_groups`.`parent_group_id` AS `parent_group_id`,`api_product_groups`.`description` AS `group_description`,`api_products`.`product_id` AS `product_id`,`api_products`.`subsystem_id` AS `subsystem_id`,`api_products`.`product` AS `product`,`api_products`.`product_level` AS `product_level`,`api_products`.`product_type` AS `product_type`,`api_products`.`basecode` AS `basecode`,`api_products`.`active` AS `active`,`api_products`.`range_limited` AS `range_limited`,`api_products`.`embargo_length` AS `embargo_length`,`api_products`.`range_start_date` AS `range_start_date_allowed`,`api_products`.`range_end_date` AS `range_end_date_allowed`,`api_products`.`parent_product_id` AS `parent_product_id`,`api_products`.`inherit_parent_metadata` AS `inherit_parent_metadata`,`api_products`.`id_type` AS `id_type`,`api_products`.`counter_service` AS `counter_service`,`api_products`.`counter_database` AS `counter_database`,`api_products`.`counter_book` AS `counter_book`,`api_products`.`counter_journal_collection` AS `counter_journal_collection`,`api_products`.`id_code_1` AS `id_code_1`,`api_products`.`id_code_2` AS `id_code_2`,`api_products`.`group_sort_order` AS `group_sort_order`,`api_products`.`hide_in_product_access` AS `hide_in_product_access`,`api_products`.`hide_in_report_list` AS `hide_in_report_list`,`api_products`.`added_by_user_id` AS `added_by_user_id`,`api_products`.`date_added` AS `date_added`,`api_products`.`modified_by_user_id` AS `modified_by_user_id`,`api_products`.`last_update` AS `last_update`,`join_groups_to_products`.`Description` AS `Description` from ((`api_product_groups` join `join_groups_to_products` on((`join_groups_to_products`.`group_id` = `api_product_groups`.`group_id`))) join `api_products` on((`api_products`.`product_id` = `join_groups_to_products`.`product_id`))) union select `api_product_groups`.`group_id` AS `group_id`,`api_product_groups`.`group_name` AS `group_name`,`api_product_groups`.`level` AS `level`,`api_product_groups`.`parent_group_id` AS `parent_group_id`,`api_product_groups`.`description` AS `group_description`,`api_products`.`product_id` AS `product_id`,`api_products`.`subsystem_id` AS `subsystem_id`,`api_products`.`product` AS `product`,`api_products`.`product_level` AS `product_level`,`api_products`.`product_type` AS `product_type`,`api_products`.`basecode` AS `basecode`,`api_products`.`active` AS `active`,`api_products`.`range_limited` AS `range_limited`,`api_products`.`embargo_length` AS `embargo_length`,`api_products`.`range_start_date` AS `range_start_date_allowed`,`api_products`.`range_end_date` AS `range_end_date_allowed`,`api_products`.`parent_product_id` AS `parent_product_id`,`api_products`.`inherit_parent_metadata` AS `inherit_parent_metadata`,`api_products`.`id_type` AS `id_type`,`api_products`.`counter_service` AS `counter_service`,`api_products`.`counter_database` AS `counter_database`,`api_products`.`counter_book` AS `counter_book`,`api_products`.`counter_journal_collection` AS `counter_journal_collection`,`api_products`.`id_code_1` AS `id_code_1`,`api_products`.`id_code_2` AS `id_code_2`,`api_products`.`group_sort_order` AS `group_sort_order`,`api_products`.`hide_in_product_access` AS `hide_in_product_access`,`api_products`.`hide_in_report_list` AS `hide_in_report_list`,`api_products`.`added_by_user_id` AS `added_by_user_id`,`api_products`.`date_added` AS `date_added`,`api_products`.`modified_by_user_id` AS `modified_by_user_id`,`api_products`.`last_update` AS `last_update`,`join_groups_to_products`.`Description` AS `Description` from ((`api_product_groups` join `join_groups_to_products` on((`join_groups_to_products`.`group_id` = `api_product_groups`.`group_id`))) join `api_products` on((`api_products`.`parent_product_id` = `join_groups_to_products`.`product_id`))) order by `group_id`;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_stat_docviews_lastweek` AS select `api_docviews`.`document_id` AS `document_id`,`api_docviews`.`type` AS `view_type`,count(0) AS `views` from `api_docviews` where (`api_docviews`.`last_update` > (now() - interval 7 day)) group by `api_docviews`.`document_id`;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_user_session_activity` AS select `api_user`.`username` AS `username`,`api_user`.`company` AS `company`,`api_user`.`enabled` AS `enabled`,`api_sessions`.`user_ip` AS `user_ip`,`api_sessions`.`connected_via` AS `connected_via`,`api_sessions`.`referrer` AS `referrer`,`api_sessions`.`session_start` AS `session_start`,`api_sessions`.`session_end` AS `session_end`,`api_endpoints`.`endpoint_url` AS `endpoint_url`,`api_session_endpoints`.`params` AS `params`,`api_session_endpoints`.`documentid` AS `documentid`,`api_session_endpoints`.`return_status_code` AS `return_status_code`,`api_session_endpoints`.`return_added_status_message` AS `return_added_status_message`,`api_session_endpoints`.`last_update` AS `last_update`,`api_session_endpoints`.`session_id` AS `session_id` from (((`api_user` join `api_sessions` on((`api_sessions`.`user_id` = `api_user`.`user_id`))) join `api_session_endpoints` on((`api_session_endpoints`.`session_id` = `api_sessions`.`session_id`))) join `api_endpoints` on((`api_endpoints`.`api_endpoint_id` = `api_session_endpoints`.`api_endpoint_id`))) order by `api_session_endpoints`.`last_update` desc;

-- ----------------------------
-- View structure for vw_user_subscriptions_products
-- ----------------------------
DROP VIEW IF EXISTS `vw_user_subscriptions_products`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_user_subscriptions_products` AS select `api_user`.`user_id` AS `user_id`,`api_user`.`username` AS `username`,`api_user`.`email_address` AS `email_address`,`api_user`.`added_date` AS `added_date`,`api_subscriptions`.`max_concurrency` AS `max_concurrency`,`api_subscriptions`.`product_id` AS `product_id`,`api_subscriptions`.`start_date` AS `start_date`,`api_subscriptions`.`end_date` AS `end_date`,`api_user`.`company` AS `company`,`api_products`.`product_description` AS `product_description`,`api_products`.`product` AS `product`,`api_products`.`product_type` AS `product_type` from ((`api_user` join `api_subscriptions` on((`api_subscriptions`.`user_id` = `api_user`.`user_id`))) join `api_products` on((`api_products`.`product_id` = `api_subscriptions`.`product_id`)));
