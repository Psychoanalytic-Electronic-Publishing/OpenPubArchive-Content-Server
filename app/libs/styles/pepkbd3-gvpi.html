<?xml version="1.0"?>
<!-- ============================================================= -->
<!--  MODULE: PEP-Web KBD3 or ARCH inst to PEP API 1.0 (GVPi) HTML -->
...
-->      <xsl:apply-templates select="@content-type"/>
  ...
  <xsl:template match="bx">
    <span class="bx peppopup bibtip" data-element="{@r}">
      <xsl:call-template name="assign-id"/>
      <xsl:apply-templates select="@content-type"/>
      <xsl:apply-templates/>
    </span>
  ...
  <xsl:template match="be">
    ...
    <div class="bib">
      ...
      <xsl:call-template name="named-anchor"/>
      <p class="bib_bib ref-content">
        <xsl:apply-templates/>
        <a class="bibx">
          <xsl:if test="@rx"> <!--possible matched reference id-->
            <xsl:attribute name="href">#/Document/<xsl:value-of select="@rx"/>            
            </xsl:attribute>
          </xsl:if>
          [&#8594;]
        </a>
