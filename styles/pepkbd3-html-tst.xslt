...
<!--  Since lxml only supports XSLT 1.0, can't use functions!-->
  
<!--  <xsl:template match="list[fn:matches(@type, '(BUL.*|DASH|DIAMOND|ASTERISK)')]">
...
<!--  <xsl:template match="list[fn:matches(@type, '(ALP|AUP|AUR|ALR|RLP|RUP|NNP|NNB|NNS)')]">
    <xsl:variable name="style">
      <xsl:choose>
...
<!--  Since lxml only supports XSLT 1.0, can't use the above functions, so do it the imperfect way!-->
  <xsl:template match="list[@type = 'ALP' or @type = 'AUP' or @type = 'AUR' or @type = 'ALR' or @type = 'RLP' or @type = 'RUP']" mode="list">
    <xsl:variable name="style">
      <xsl:choose>
        <xsl:when test="@type = 'ALP'">lower-alpha</xsl:when>
        <xsl:when test="@type = 'AUP'">upper-alpha</xsl:when>
    ...
    <xsl:template match="list[@type = 'NNP' or @type = 'NNB' or @type = 'NNS' or @type = 'DASH' or @type = 'DIAMOND' or @type = 'ASTERISK']" mode="list">
    <xsl:variable name="style">
      <xsl:choose>
        <xsl:when test="@type = 'NNP'">number-period</xsl:when>
        <xsl:when test="@type = 'NNB'">number-both-parens</xsl:when>
