...
CREATE TABLE `articles` (
  ...
  UNIQUE KEY `Primary Key` (`articleID`) USING BTREE,
...
CREATE TABLE `fullbiblioxml` (
  ...
  KEY `RefersTo` (`rxCode`) USING BTREE,
...
CREATE TABLE `ip_addresses` (
  ...
  CONSTRAINT `ip_addresses_ibfk_1` FOREIGN KEY (`user_id`, `subsystem_id`) REFERENCES `user_subsystem` (`user_id`, `subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `product` (
  ...
  CONSTRAINT `product_ibfk_1` FOREIGN KEY (`subsystem_id`) REFERENCES `subsystem` (`subsystem_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `referrer_urls` (
  ...
  KEY `idxUserId` (`user_id`) USING BTREE,
  KEY `FK_referrer_urls_user_subsystem` (`user_id`,`subsystem_id`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=119128 DEFAULT CHARSET=utf8;
...
-- ----------------------------
-- View structure for vw_cited_in_all_years
-- ----------------------------
DROP VIEW IF EXISTS `vw_cited_in_all_years`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_cited_in_all_years` AS select `fullbiblioxml`.`rxCode` AS `rxCode`,count(0) AS `countAll` from (`fullbiblioxml` join `articles`) where ((`fullbiblioxml`.`articleID` = `articles`.`articleID`) and (`fullbiblioxml`.`rxCode` is not null) and (`fullbiblioxml`.`rxCode` <> '') and (`articles`.`year` > (year(now()) - 20)) and (substr(`fullbiblioxml`.`rxCode`,1,3) not in ('ZBK','IPL','SE.','GW.'))) group by `fullbiblioxml`.`rxCode` order by `countAll` desc ;

-- ----------------------------
-- View structure for vw_cited_in_last_10_years
-- ----------------------------
DROP VIEW IF EXISTS `vw_cited_in_last_10_years`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_cited_in_last_10_years` AS select `fullbiblioxml`.`rxCode` AS `rxCode`,count(0) AS `count10` from (`fullbiblioxml` join `articles`) where ((`fullbiblioxml`.`articleID` = `articles`.`articleID`) and (`fullbiblioxml`.`rxCode` is not null) and (`articles`.`year` > (year(now()) - 10)) and (substr(`fullbiblioxml`.`rxCode`,1,3) not in ('ZBK','IPL','SE.','GW.'))) group by `fullbiblioxml`.`rxCode` order by `count10` desc ;

-- ----------------------------
-- View structure for vw_cited_in_last_20_years
-- ----------------------------
DROP VIEW IF EXISTS `vw_cited_in_last_20_years`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_cited_in_last_20_years` AS select `fullbiblioxml`.`rxCode` AS `rxCode`,count(0) AS `count20` from (`fullbiblioxml` join `articles`) where ((`fullbiblioxml`.`articleID` = `articles`.`articleID`) and (`fullbiblioxml`.`rxCode` is not null) and (`articles`.`year` > (year(now()) - 20)) and (substr(`fullbiblioxml`.`rxCode`,1,3) not in ('ZBK','IPL','SE.','GW.'))) group by `fullbiblioxml`.`rxCode` order by `count20` desc ;

-- ----------------------------
-- View structure for vw_cited_in_last_5_years
-- ----------------------------
DROP VIEW IF EXISTS `vw_cited_in_last_5_years`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_cited_in_last_5_years` AS select `fullbiblioxml`.`rxCode` AS `rxCode`,count(0) AS `count5` from (`fullbiblioxml` join `articles`) where ((`fullbiblioxml`.`articleID` = `articles`.`articleID`) and (`fullbiblioxml`.`rxCode` is not null) and (`articles`.`year` > (year(now()) - 5)) and (substr(`fullbiblioxml`.`rxCode`,1,3) not in ('ZBK','IPL','SE.','GW.'))) group by `fullbiblioxml`.`rxCode` order by `count5` desc ;

-- ----------------------------
-- View structure for vw_document_views_crosstab
-- ----------------------------
DROP VIEW IF EXISTS `vw_document_views_crosstab`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_document_views_crosstab` AS select `r0`.`documentid` AS `documentid`,max(`r0`.`last_viewed`) AS `last_viewed`,`r1`.`views` AS `lastweek`,`r2`.`views` AS `lastmonth`,`r3`.`views` AS `last6months`,`r5`.`views` AS `last12months`,`r4`.`views` AS `lastcalyear` from (((((((select distinct `opascentral`.`api_session_endpoints`.`documentid` AS `documentid`,`opascentral`.`api_session_endpoints`.`last_update` AS `last_viewed` from `opascentral`.`api_session_endpoints`)) `r0` left join `opascentral`.`vw_docviews_lastweek` `r1` on((`r1`.`documentid` = `r0`.`documentid`))) left join `opascentral`.`vw_docviews_lastmonth` `r2` on((`r2`.`documentid` = `r0`.`documentid`))) left join `opascentral`.`vw_docviews_lastsixmonths` `r3` on((`r3`.`documentid` = `r0`.`documentid`))) left join `opascentral`.`vw_docviews_lastcalyear` `r4` on((`r4`.`documentid` = `r0`.`documentid`))) left join `opascentral`.`vw_docviews_last12months` `r5` on((`r5`.`documentid` = `r0`.`documentid`))) where ((`r0`.`documentid` is not null) and (`r0`.`documentid` <> 'None')) group by `r0`.`documentid` ;

-- ----------------------------
-- View structure for vw_docviews_last12months
-- ----------------------------
DROP VIEW IF EXISTS `vw_docviews_last12months`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_docviews_last12months` AS select `api_session_endpoints`.`documentid` AS `documentid`,count(0) AS `views` from `api_session_endpoints` where (`api_session_endpoints`.`last_update` > (now() - interval 12 month)) group by `api_session_endpoints`.`documentid` ;
...
-- ----------------------------
-- View structure for vw_reference_citation_crosstab
-- ----------------------------
DROP VIEW IF EXISTS `vw_reference_citation_crosstab`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_reference_citation_crosstab` AS select `r0`.`articleID` AS `documentid`,`r1`.`count5` AS `count5`,`r2`.`count10` AS `count10`,`r3`.`count20` AS `count20`,`r4`.`countAll` AS `countAll` from ((((((select distinct `opascentral`.`fullbiblioxml`.`articleID` AS `articleID`,`opascentral`.`fullbiblioxml`.`internalID` AS `internalID`,`opascentral`.`fullbiblioxml`.`fullReference` AS `fullReference`,`opascentral`.`fullbiblioxml`.`rxCode` AS `rxCode` from `opascentral`.`fullbiblioxml`)) `r0` left join `opascentral`.`vw_cited_in_last_5_years` `r1` on((`r1`.`rxCode` = `r0`.`rxCode`))) left join `opascentral`.`vw_cited_in_last_10_years` `r2` on((`r2`.`rxCode` = `r0`.`rxCode`))) left join `opascentral`.`vw_cited_in_last_20_years` `r3` on((`r3`.`rxCode` = `r0`.`rxCode`))) left join `opascentral`.`vw_cited_in_all_years` `r4` on((`r4`.`rxCode` = `r0`.`rxCode`))) where ((`r0`.`rxCode` is not null) and (`r0`.`rxCode` <> 'None')) group by `r0`.`rxCode` order by `r3`.`count20` desc ;
...
-- ----------------------------
-- View structure for vw_stat_cited
-- ----------------------------
DROP VIEW IF EXISTS `vw_stat_cited`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_stat_cited` AS select `fullbiblioxml`.`articleID` AS `articleID`,`fullbiblioxml`.`internalID` AS `internalID`,`fullbiblioxml`.`fullReference` AS `fullReference`,`fullbiblioxml`.`rxCode` AS `rxCode`,`vw_cited_in_last_20_years`.`count20` AS `count20`,`vw_cited_in_all_years`.`countAll` AS `countAll` from ((((`vw_cited_in_all_years` left join `fullbiblioxml` on((`fullbiblioxml`.`articleID` = `vw_cited_in_all_years`.`rxCode`))) left join `vw_cited_in_last_20_years` on((`vw_cited_in_all_years`.`rxCode` = `vw_cited_in_last_20_years`.`rxCode`))) left join `vw_cited_in_last_10_years` on((`vw_cited_in_last_20_years`.`rxCode` = `vw_cited_in_last_10_years`.`rxCode`))) left join `vw_cited_in_last_5_years` on((`vw_cited_in_last_10_years`.`rxCode` = `vw_cited_in_last_5_years`.`rxCode`))) group by `fullbiblioxml`.`articleID` ;

-- ----------------------------
-- View structure for vw_stat_most_viewed
-- ----------------------------
DROP VIEW IF EXISTS `vw_stat_most_viewed`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_stat_most_viewed` AS select `vw_document_views_crosstab`.`documentid` AS `documentid`,`vw_document_views_crosstab`.`last_viewed` AS `last_viewed`,coalesce(`vw_document_views_crosstab`.`lastweek`,0) AS `lastweek`,coalesce(`vw_document_views_crosstab`.`lastmonth`,0) AS `lastmonth`,coalesce(`vw_document_views_crosstab`.`last6months`,0) AS `last6months`,coalesce(`vw_document_views_crosstab`.`last12months`,0) AS `last12months`,coalesce(`vw_document_views_crosstab`.`lastcalyear`,0) AS `lastcalyear`,`opascentral`.`articles`.`hdgauthor` AS `hdgauthor`,`opascentral`.`articles`.`hdgtitle` AS `hdgtitle`,`opascentral`.`articles`.`srctitleseries` AS `srctitleseries`,`opascentral`.`articles`.`publisher` AS `publisher`,`opascentral`.`articles`.`jrnlcode` AS `jrnlcode`,`opascentral`.`articles`.`year` AS `pubyear`,`opascentral`.`articles`.`vol` AS `vol`,`opascentral`.`articles`.`pgrg` AS `pgrg`,`opascentral`.`articles`.`preserve` AS `preserve`,`opascentral`.`articles`.`filename` AS `filename`,`opascentral`.`articles`.`bktitle` AS `bktitle`,`opascentral`.`articles`.`bkauthors` AS `bkauthors`,`opascentral`.`articles`.`xmlref` AS `xmlref`,`opascentral`.`articles`.`authorMast` AS `authorMast`,`opascentral`.`articles`.`issue` AS `issue`,`opascentral`.`articles`.`updated` AS `updated` from (`opascentral`.`vw_document_views_crosstab` join `opascentral`.`articles` on((convert(`opascentral`.`articles`.`articleID` using utf8) = `vw_document_views_crosstab`.`documentid`))) ;
